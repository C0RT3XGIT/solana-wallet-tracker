...
const getTransactionDetailsBySignature = asyncHandler(async (req, res) => {
  ...
  const transactionInfo = await transactionsService.transactionDetailsBySignature(signature);
...
const getTokenTransfersBySignature = asyncHandler(async (req, res) => {
  try {
    ...
    const transactionInfo = await transactionsService.tokenTransfersBySignature(signature, publicKey);
    res.json(transactionInfo);
  } catch (err) {
...
const getTokenDetailsByMint = asyncHandler(async (req, res) => {
  const { mint } = req.body;
  const publicKey = new PublicKey(mint);
  const tokenDetails = await transactionsService.getTokenDetails(publicKey);
  res.json(tokenDetails);
})

const getTokenAndSolBalanceBySignature = asyncHandler(async (req, res) => {
  const { signature} = req.body;
  const tokenAndSolBalance = await transactionsService.tokenAndSolBalanceBySignature(signature);
